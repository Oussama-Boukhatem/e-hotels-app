...
-- =========================================
-- Trigger 3 (Optional but recommended):
-- Ensure a renting converted from a booking
-- matches the same customer and room
-- =========================================

CREATE OR REPLACE FUNCTION validate_renting_booking_match()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    IF NEW.BookingID IS NOT NULL THEN
        ...
        INTO booking_customer_id, booking_room_id
        FROM Booking
        WHERE BookingID = NEW.BookingID;

        IF booking_customer_id IS NULL THEN
            RAISE EXCEPTION 'Invalid BookingID: referenced booking does not exist.';
        END IF;

        IF NEW.CustomerID <> booking_customer_id THEN
            RAISE EXCEPTION 'Renting error: CustomerID does not match the linked booking.';
        END IF;

        IF NEW.RoomID <> booking_room_id THEN
            RAISE EXCEPTION 'Renting error: RoomID does not match the linked booking.';
        END IF;
    END IF;
...
CREATE TRIGGER trg_validate_renting_booking_match
BEFORE INSERT OR UPDATE ON Renting
FOR EACH ROW
EXECUTE FUNCTION validate_renting_booking_match();